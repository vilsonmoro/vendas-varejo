...
async function buscarProdutos(nomeProduto) {
    const response = await fetch(`http://localhost:8080/buscar?nomeProduto=${encodeURIComponent(nomeProduto)}`);
    if (!response.ok) {
...
async function cadastrarPedidoProduto(pedidoProdutoData) {
    ...
    const response = await fetch('http://localhost:8080/pedidoproduto', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
