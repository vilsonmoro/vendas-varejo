...
async function buscarProdutos(nomeProduto) {
    const response = await fetch(`http://localhost:8080/search?nomeProduto=${encodeURIComponent(nomeProduto)}`);
    if (!response.ok) {
...
async function cadastrarVenda() {
    ...
    const vendaResponse = await fetch('http://localhost:8080/venda', {
        method: 'POST',
        headers: {
...
async function cadastrarProdutoVenda(vendaId) {
    ...
    await Promise.all(produtosVenda.map(produto => {
        return fetch('http://localhost:8080/produtovenda', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
